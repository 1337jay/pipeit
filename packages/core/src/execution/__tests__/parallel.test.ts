...
describe('submitParallel', () => {
    ...
    it('should throw error for empty endpoints', async () => {
        await expect(submitParallel({ endpoints: [], transaction: 'base64tx' })).rejects.toThrow(ParallelSubmitError);
        await expect(submitParallel({ endpoints: [], transaction: 'base64tx' })).rejects.toThrow(
...
describe('submitToRpc', () => {
    ...
    it('should submit transaction and return signature', async () => {
        ...
        const signature = await submitToRpc('https://rpc.example.com', 'base64tx');
    ...
    it('should throw on RPC error response', async () => {
        ...
        await expect(submitToRpc('https://rpc.example.com', 'base64tx')).rejects.toThrow(
    ...
    it('should throw on HTTP error', async () => {
        ...
        await expect(submitToRpc('https://rpc.example.com', 'base64tx')).rejects.toThrow('429');
    });
